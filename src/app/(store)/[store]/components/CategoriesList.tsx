...
export default function CategoriesList({
  ...
  const displayedItems = showAllFood ? categories : categories.slice(0, 10);
  ...
  return (
    <div className="space-y-6">
            ...
            {item.products?.slice(0, 5).map((product) => (
              ...
              >
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">{storeData?.settings.currency} {product.price}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      {!showAllFood && categories.length > 10 && (
        <div className="text-center">
          <Button
            onClick={() => setShowAllFood(true)}
            className="bg-white border border-gray-200 text-accent-foreground hover:bg-gray-50"
          >
            Show All Categories ({categories.length - 10} more)
          </Button>
        </div>
      )}
    </div>
